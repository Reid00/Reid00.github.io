...
![image-20191201222429886](C:\Users\v-baoz\AppData\Roaming\Typora\typora-user-images\image-20191201222429886.png)
...
![image-20191201222456542](C:\Users\v-baoz\AppData\Roaming\Typora\typora-user-images\image-20191201222456542.png)
...
![image-20191201222555831](C:\Users\v-baoz\AppData\Roaming\Typora\typora-user-images\image-20191201222555831.png)

 

- 平方损失函数：用来描述回归问题，用来表示连续性变量，为预测值与真实值差值的平方。（误差值越大、惩罚力度越强，也就是对差值敏感）

![image-20191201222620710](C:\Users\v-baoz\AppData\Roaming\Typora\typora-user-images\image-20191201222620710.png)

 

- 绝对损失函数：用在回归模型，用距离的绝对值来衡量

![image-20191201222638485](C:\Users\v-baoz\AppData\Roaming\Typora\typora-user-images\image-20191201222638485.png)
...
- 对数损失函数：是预测值Y和条件概率之间的衡量。事实上，该损失函数用到了极大似然估计的思想。P(Y|X)通俗的解释就是：在当前模型的基础上，对于样本X，其预测值为Y，也就是预测正确的概率。由于概率之间的同时满足需要使用乘法，为了将其转化为加法，我们将其取对数。最后由于是损失函数，所以预测正确的概率越高，其损失值应该是越小，因此再加个负号取个反。
 ...
 ![image-20191201222700230](C:\Users\v-baoz\AppData\Roaming\Typora\typora-user-images\image-20191201222700230.png)
